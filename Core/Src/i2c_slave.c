...
/*******************************************************************/
map_t map;
const uint16_t I2C1_RAM_SIZE = sizeof(CONTROL_BOARD_REGS_t);
uint8_t i2c1_ram_adr=0;
//uint8_t i2c1_ram[I2C1_RAM_SIZE]; //for test only
//extern volatile uint8_t ADCBuffer_2[16];

/*******************************************************************/
uint8_t get_i2c1_ram(uint8_t adr) {
	//ADC data
	/*if ((ADC_ADDR_START <= adr) & (adr < ADC_ADDR_START + ADC_CHANNELS*2)) {
		return ADCBuffer_2[adr - ADC_ADDR_START];
	}
	else {*/
		// Other addresses
		return map.i2c1_ram[adr];
	//}
}

void set_i2c1_ram(uint8_t adr, uint8_t val) {
	if(adr > I2C1_RAM_SIZE){return;}
	
	map.i2c1_ram[adr] = val;
	return;
}
...
void I2C1_Slave_init(void) {
	...
	memset(map.i2c1_ram, 0, I2C1_RAM_SIZE);//clear i2c buffer
}
/*******************************************************************/
